...
import { ArrowRight, BookOpen, Check, Compass, Mountain, RefreshCw, Sparkles, WifiOff } from 'lucide-react';
...
type PageId = 'home' | 'directions' | 'saved' | 'profile';
const pages: PageId[] = ['home', 'directions', 'saved', 'profile'];

export default function App() {
  const { data, action, error, refresh, connection, lastSyncAt } = useAppData();
  const [page, setPage] = useState<PageId>(() => {
    try { const stored = sessionStorage.getItem('pickoneq-page') as PageId | null; return stored && pages.includes(stored) ? stored : 'home'; }
    catch { return 'home'; }
  });
  ...
  const [splash, setSplash] = useState(() => {
    try { return window.matchMedia('(max-width: 640px)').matches && sessionStorage.getItem('pickoneq-splash') !== 'seen'; }
    catch { return window.matchMedia('(max-width: 640px)').matches; }
  });
  const closeSplash = () => {
    try { sessionStorage.setItem('pickoneq-splash', 'seen'); } catch { /* session storage may be unavailable */ }
    setSplash(false);
  };
  useEffect(() => {
    if (!splash) return;
    const timer = window.setTimeout(closeSplash, window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 200 : 2100);
    return () => clearTimeout(timer);
  }, [splash]);
  useEffect(() => { if (!toast) return; const t = window.setTimeout(() => setToast(''), 4500); return () => clearTimeout(t); }, [toast]);
  useEffect(() => {
    window.scrollTo(0, 0);
    try { sessionStorage.setItem('pickoneq-page', page); } catch { /* session storage may be unavailable */ }
  }, [page]);
  ...
  return <div className={`app-shell ${page === 'home' && data.state.stage === 'question' ? 'mobile-discovery' : ''}`}>
    {splash && <div className="launch-screen" role="status" aria-label="拾一问，正在开启探索"><div className="launch-halo" /><img src="/app-icon.png" alt="拾一问图标" /><h1>拾一问</h1><p>一个问题，打开更大的世界</p><button onClick={closeSplash}>跳过</button></div>}
    ...
    <aside className="sidebar"><a href="#" className="brand" onClick={e => { e.preventDefault(); setPage('home'); }}><Mountain className="brand-mark" strokeWidth={1.1} /><span>拾一问<small>PickOneQ</small></span></a><nav aria-label="主导航">{navItems.map(({ id, label, icon: Icon }) => <button type="button" className={page === id ? 'active' : ''} aria-current={page === id ? 'page' : undefined} key={id} onClick={() => setPage(id as PageId)}><Icon strokeWidth={1.6} fill={id === 'home' && page === id ? 'currentColor' : 'none'} /><span>{label}</span></button>)}</nav><div className="sidebar-poem">好奇一问，<br />让世界更大一点。<i /></div></aside>
    <main className="main"><header className="topbar"><div><h2>{page === 'home' ? '自由探索' : page === 'directions' ? '我的方向' : page === 'saved' ? '拾遗' : '我的世界'}</h2><p>在好奇中，遇见更大的世界。</p></div><button className="mode-indicator" onClick={() => setSettings(true)}>{data.credential.configured ? <Sparkles size={21} /> : <BookOpen size={21} />}<span>{data.credential.configured ? data.generation.busy ? '正在准备新问题' : '好奇，正在生长' : '精选示例'}<small>{data.credential.configured ? data.generation.busy ? data.generation.phase === 'verifying' ? '正在独立核验' : '正在生成问题与解释' : `已有 ${data.generation.ready} 道 AI 新题` : '从一个问题开始'}<ArrowRight size={13} /></small></span></button></header>
      {connection === 'offline' && <div className="connection-banner" role="alert"><WifiOff size={17} /><span><strong>本地服务暂时断开</strong>当前页面和输入仍会保留{lastSyncAt ? ` · 上次同步 ${new Date(lastSyncAt).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })}` : ''}</span><button onClick={() => void refresh()}><RefreshCw size={15} />重新连接</button></div>}
      <div className="main-inner">{page === 'home' ? <QuestionView {...props} settings={() => setSettings(true)} directions={() => setPage('directions')} /> : page === 'directions' ? <Directions {...props} open={open} /> : page === 'saved' ? <Saved {...props} open={open} explore={() => setPage('home')} /> : <Profile {...props} showSettings={() => setSettings(true)} />}</div>
