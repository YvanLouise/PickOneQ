...
export function Conversation({ q, data, action }: { q: Question; data: Snapshot; action: Action }) {
  ...
  return <section className="conversation">
    ...
    {opened && <>
      ...
      <form onSubmit={e => { e.preventDefault(); void send(input); }}><label htmlFor={`chat-${q.id}`}>你还想弄清什么？</label><textarea id={`chat-${q.id}`} value={input} maxLength={8000} onChange={e => setInput(e.target.value)} placeholder="可以追问原因，也可以试着换一个条件。" /><div className="form-actions"><small className="muted">输入保存在本机</small>{active ? <button type="button" className="button secondary compact" onClick={() => action(`conversation/${session!.id}/cancel`).catch(e => setError(e.message))}>停止等待</button> : <button className="button primary compact" disabled={sending || !input.trim() || !data.credential.configured}>{sending ? '正在发送…' : data.credential.configured ? '发送问题' : '请先配置模型'}</button>}</div></form>
    </>}
